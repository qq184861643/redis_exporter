...
func main() {
	...
	if *isDebug {
		//log.Infof("flag v value： %s", flag.Lookup("v").Value)
	...
	log.V(5).Infof("Configured redis addr: %#v", *redisAddr)
	if *tlsServerCertFile != "" && *tlsServerKeyFile != "" {
		log.V(5).Infof("Bind as TLS using cert %s and key %s", *tlsServerCertFile, *tlsServerKeyFile)
